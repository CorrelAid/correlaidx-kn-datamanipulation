...
In R, `if` can only check one value at a time $\rightarrow$ we need *vectorized* functions! You have seen `if_else()` earlier :
...
`modelsummary` can also work with a `list()` of models:

```{r}
#| eval: true
models <- list(
  "naive" = model,
  "fixed-effects" = model_fe
)
...
Data on Himalaya-expeditions (click [here](https://github.com/rfordatascience/tidytuesday/blob/master/data/2020/2020-09-22/readme.md) for more details):
...
* We want to find out what predicts survival or death of expedition members.

## Suggestions

* Join information from `peaks` (e.g. height, whether it has been climbed before etc.) and `expeditions` (e.g. number of expedition members, number of hired members) into the `members`-data frame using a `left_join()`:

```{r}
# For example:
members |> left_join(peaks, by = "peak_id")
# ...
```

...then go from here ;) Below is a **suggestion** of what you could do for data prep (click to expand).
...
full <- 
  ...
  left_join(expeditions, by = "expedition_id") |> # ignore the warning for now
  mutate(
    climbed_before = if_else(climbing_status == "Unclimbed", 1, 0),
    male = if_else(sex == "M", 1, 0),
    across(c(solo, success, died), as.integer)
  )
```
