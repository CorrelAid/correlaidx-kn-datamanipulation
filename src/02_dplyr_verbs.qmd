...
If you are working with strings, use `stringr`:
...
bigfoot |> 
  mutate(
    location_details = str_to_lower(location_details),
    in_forest = str_detect(location_details, "forest"),
    # RegEx also work:
    report_id = str_extract(title, "\\d+"),
    # This is also valid:
    hiking = observed |> str_to_lower() |> str_detect("hike|hiking")
  )
```
