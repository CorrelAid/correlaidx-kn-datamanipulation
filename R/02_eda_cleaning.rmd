...
If we are lazy, we can just convert the unknowns to `NA` (missing value, "Not Available") (*Explain `if_else()`*):

```{r}
bf |> 
  mutate(season = if_else(season == "Unknown", NA, season))
```
...
```{r}
bf <- bf |> 
  mutate(reliable = if_else(classification == "Class A", 1, 0))
```
