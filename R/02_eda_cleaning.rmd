...
## Quick rundown: Setting up a reproducible project 

1. Create a folder for your project in a proper location. Create separate folders for your code and your data inside of the project folder:

<center>
![](../assets/project_folder.png){width=500px}
</center>

2. In RStudio, in the top left, go to File > New Project... . In the popup, select "Existing directory" & select your directory:

<center>
![](../assets/project_popup.png){width=500px}
</center>

<center>
![](../assets/select_project.png){width=600px}
</center>

This will put you into the project. To select/change the current project, use the little Project-button in the top-right:

<center>
![](../assets/project_view.png){width=800px}
</center>

Projects are a clean way of separating multiple different... well, projects & they allow you to use relative paths from where the `.Rproj`-file resides using the `here`-package (for example, when inside your project, to read a file from "data", the path would just be "data/file"). Double-clicking a `.Rproj`-file should also open the project in RStudio. Projects also allow you to ship your project self-contained: anyone can open the project and reproduce, no need to deal with file paths etc.
...
One thing we have not discussed yet is string manipulation. Look at the y-axis labels: We may want to format the names a bit nicer. We can do this by modifying the strings (names) in the "term"-column:
...
For everything with strings, use [`stringr`](https://stringr.tidyverse.org/):
...
tidy_model <- 
  ...
  mutate(
    term = 
      term |> # take term, *and then...*
      # The syntax of stringr is quite intuitive:
      str_replace("_", " ") |> 
      str_to_title() |> 
      str_replace("Uv", "UV")
  )

tidy_model |> select(term)
```
...
tidy_model |> 
  ...
  geom_vline(xintercept = 0, color = "darkgrey") +
  geom_pointrange(aes(xmin = lower, xmax = upper)) +
  labs(x = "Coefficient estimate", y = "", title = "Predictors of reliable bigfoot sightings")
```
