...
Now you have little data frames inside your data frame! We can now do operations for all these separate data frames separately, but at once; for example fit a model to every subset:
...
fitted <- 
  ...
  mutate(model = map(data, \(df) lm(lifeExp ~ log10(gdpPercap) + factor(year), data = df)))

fitted
```

This adds another list column holding the models for every continent. Now we could look at where the model has the best fit (here by $R^2$):

```{r}
fitted |> 
  mutate(r2 = map_dbl(model, \(mod) summary(mod)$r.squared)) |> 
  arrange(desc(r2))
```
